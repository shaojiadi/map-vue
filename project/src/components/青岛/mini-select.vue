      ...
      <div class="option-popup" v-if="optionShow">
        <ul class="option" ref="selectScroll">
          <li class="option-item" v-for="(item,index) in optionList" :key="index"
              :class="{'active':labelId === item[useTemplate.value]}" @click.stop="changeOption(item,index)"
              ref="option-item">
            <div class="option-content">
              <i class="circle" v-if="isNeedCircle"></i>
              <p class="ellipsis" :title="item[useTemplate.label]">{{item[useTemplate.label]}}</p>
            </div>
          </li>
        </ul>
      </div>
