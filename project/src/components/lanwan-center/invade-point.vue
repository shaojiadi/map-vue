...
<!-- 巡检画面 巡检记录 -->
  <ul class="camera">
    <!-- <li
      ...
      :ref="'camera-' + (index + 1)"
    > -->
      <div class="f-camera-popup" ref="camera-popup" v-if="cameraId === patrolList[0].id">
        <div class="f-camera-popup-header">
          <div class="f-camera-popup-btn">
            <ul class="tab-list">
              <li v-for="(item, index) in tabList" :key="index" @click="isInPut = item.isIn" class="show-item" :class="item.isIn == isInPut ? 'active' : ''">{{ item.name }}</li>
            </ul>
        ...
        <div class="f-camera-popup-content">
          <div class="f-camera-popup-video" v-if="isInPut === 0">
            ...
            <!-- <v-hikVideo :activeCameraIndexCode="activeCameraIndexCode"></v-hikVideo> -->
            <p class="popup-content-bottom">
              监测地点：<span>{{ patrolList[0].local }}</span>
            </p>
          </div>
...
let waiweimonitoList = () =>
  axios.get(`/mortar/screen/waiweimonitoList?regionIndexCode=` + "46ef687b-e0ed-4490-bfb1-b3d0dabaeec4");
